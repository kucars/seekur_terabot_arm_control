...
int main(int argc, char **argv)
{
  ...
  ArRobotConnector robotConnector(&parser, &robot1);
 ...
 // arm.setInputDeviceConnection(&masterCon);
  ...
  arm1.reset();
  arm1.enableArm();
  arm1.setAllJointSpeeds(defaultJointSpeed);
  ...
  //arm1.moveArm(forwardReady);
  positions= arm1.getArmPos();
  ...
  robot1.lock();
  //TerabotDemoMode terabotDemoMode(&robot1, &arm1);
  //ArModeUnguardedTeleop unguardedTeleopMode(&robot1, "unguarded teleop", 'u', 'U');
  //ArModeTeleop teleopMode(&robot1, "teleop", 't', 'T');
  //ArModeLaser laserMode(&robot1, "laser", 'l', 'L');
  //ArModeConfig configMode(&robot, "show robot config", 'o', 'O');
  //ArModeCommand commandMode(&robot1, "direct robot commands", 'd', 'D');
  //terabotDemoMode.activate();

  robot1.enableMotors(); // teleop modes don't
  
  sensor_msgs::JointState joints;
  ros::NodeHandle n; 

  ros::Rate loop_rate(10);
  ros::Publisher joint_pub = n.advertise<sensor_msgs::JointState>("/joint_states", 50);//1000
...
while(ros::ok())
  {
	...
	//for( int i=0; i<5 ;i++)
	//joints.position.clear();
	  //joints.position.clear();joints.position.clear();joints.position.clear();
          //joints.position.clear();joints.position.clear();
         ros::spinOnce();
         loop_rate.sleep();
        std::cout<<"loopend\n";
   }
